...
> **v5.0+ 更新说明**：本文档已更新至v5.0+，补充10-Agent Pipeline架构层。完整功能清单参见 [COMPLETENESS_ANALYSIS.md](./COMPLETENESS_ANALYSIS.md)。

## 一、系统概述

### 1.1 项目定位
AI小说工坊是一个支持100万字以上长篇小说智能生成的本地优先应用，桌面端使用 Tauri + SQLite，浏览器端保留 IndexedDB 备用运行模式，支持全自动和半自动两种创作模式，用户可随时干预调整。

### 1.2 核心特性
- 支持所有类型小说创作，用户自定义篇幅
- 10-Agent Pipeline 全流程自动化（规划→组装→写作→标准化→审计→修订→状态沉淀→分析→伏笔→验证）
- 17维质量审计 + 多轮修订循环 + 快照回滚
- 分层AI模型策略（规划用高智商模型、写作用性价比模型，带故障转移容错）
- 批量生成和实时生成双模式
- 完整的设定系统（世界观、人物、大纲）
- 智能记忆系统（支持100万字+上下文，基于 Middleware Pipeline 构建）
- 伏笔追踪（种植/推进/回收/提及验证）+ 叙事控制（7段结构）
- 质量检查与自动修复 + 敏感词检测（含开关）
- 模板系统（可导出分享）+ 14种类型配置
- Tauri 桌面端基于原子化 API IPC 更新 SQLite，彻底消除 OOM 与吃书风险；浏览器端通过 IndexedDB 支撑轻量体验
- 编译期运行时边界：Vite 注入 `__APP_IS_TAURI__`，前端统一通过 `isWebRuntime()` 选择存储与安全策略，不信任可伪造的浏览器全局变量
- 暗色优先现代工作台：全局 Design System 统一项目首页、写作工作区、章节编辑器、AI 助手和统计面板视觉
...
│  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘    │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │  Design System: --ds-* 令牌 / Element Plus 变量桥接 / dark-light root │  │
│  │  Notion/Linear 风格 ProjectList + CSS Grid ProjectEditor 工作区       │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐    │
...
│  └──────────────────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │                   10-Agent Pipeline (v5.0+ 新增)                       │  │
│  │  Planner→Composer→Writer→Normalizer→Auditor→Reviser→Settler→Analyzer→HookPromoter→Validator │  │
│  │  - 17维审计  - 审计-修订循环  - 快照回滚  - 批量续写调度               │  │
│  │  - 伏笔追踪  - 节奏分析  - 叙事控制  - 敏感词检测  - 状态沉淀          │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │                        记忆管理服务                                    │  │
...
│                                                                              │
│  ┌────────────────┐  ┌────────────────┐  ┌────────────────┐                │
│  │   SQLite DB    │  │   IndexedDB    │  │ instant-distance │               │
│  │  (Tauri桌面端) │  │  (浏览器备用)  │  │   (Rust HNSW)   │                │
│  └────────────────┘  └────────────────┘  └────────────────┘                │
│                                                                              │
│  ┌────────────────┐  ┌────────────────┐  ┌────────────────┐                │
│  │   配置存储     │  │   缓存系统     │  │   备份系统     │                │
