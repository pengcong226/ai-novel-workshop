...
8.  **`save_entity`** / **`save_state_event`**
    *   **用途**：操作 V5 Entity & StateEvent 系统。`save_entity` 创建或更新实体（角色/地点/物品等），`save_state_event` 记录状态变更事件（如角色位置移动、受伤、获得物品等）。
...
> 3. 如果某个角色的状态（受伤、获得神器、换地图）发生了改变，调用 `save_state_event` 更新实体状态。
> 
> 请自主循环执行以上步骤，直到我喊停为止。

通过这种方式，原本需要人类在 UI 上反复点击”读取前文 -> 生成 -> 抽取人物 -> 更新状态”的繁琐流程，将完全由外置 Agent 自主规划并瞬间完成。
