...
const logger = getLogger('utils:aiAnalyzer')
...
async function callAI(
  ...
  if (config.provider === 'claude') {
    ...
    logger.info('[AI分析] 直接调用自定义API:', apiURL)
  } else {
    throw new Error('未配置AI API密钥或地址')
  }

  try {
    logger.info('[AI分析] 调用API:', apiURL)
    ...
    if (!response.ok) {
      ...
      logger.error('[AI分析] API错误响应:', errorText)
    ...
    if (config.provider === 'claude') {
      ...
      logger.info('[AI分析] Claude响应长度:', text.length)
      return text
    } else {
      const text = data.choices?.[0]?.message?.content || ''
      logger.info('[AI分析] OpenAI响应长度:', text.length)
      return text
    }
  } catch (error) {
    logger.error('[AI分析] API调用失败:', error)
