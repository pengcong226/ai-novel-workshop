/**
 * 写作方法论注入 (Writing Methodology Injection)
 *
 * 将完整的写作方法论文本注入 Writer/Composer 的 prompt 中。
 * 包含去AI味正反例、六步走心理分析、配角设计、代入感支柱等。
 *
 * 借鉴 InkOS writing-methodology.ts，适配工坊的 prompt 注入方式。
 */

/**
 * 构建写作方法论段落（用于注入 system prompt）
 * @param language 语言
 * @returns 方法论文本
 */
export function buildWritingMethodologySection(language: 'zh' | 'en' = 'zh'): string {
  if (language === 'en') {
...
function buildEnglishMethodology(): string {
  return `# Writing Methodology Reference
...
2. **Concrete/visual**: Specific scenes readers can visualize
3. **Familiarity**: Scenes readers have lived through carry natural immersion
4. **Resonance**: Protagonist's struggle must feel universal
...
2. **Progressive escalation**: Stack bad things — each layer worse than the last
